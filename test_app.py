...
def test_pred_Bad_risk():
    ...
    with TestClient(app) as client:
        ...
        # asserting the correct response is received
        assert response.status_code == 200
        assert response.json() == {"credit_risk": "Bad Risk", "run_Time_timestamp":str(datetime.datetime.now().replace(microsecond=0))}
...
def test_pred_Good_risk():
    ...
    with TestClient(app) as client:
        ...
        # asserting the correct response is received
        assert response.status_code == 200
        assert response.json() == {"credit_risk": "Good Risk", "run_Time_timestamp":str(datetime.datetime.now().replace(microsecond=0))}
